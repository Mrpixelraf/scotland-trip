...
**第一次加入请先用 [JOIN.md](../JOIN.md)**：里面包含访问检查、已有邀请的接入办法，以及按轮次收集时间、预算、住宿和活动倾向的问题清单。仓库已公开，阅读无需邀请；回答先留在本人对话，只同步本人明确同意公开的信息。

对外转发：[无需仓库权限即可读取的接入说明](https://gist.github.com/Mrpixelraf/61a46535507361a21e009e5821afede7)。这是一份未公开列出的 Gist，持链接者可读，只保留通用内容。

## 成员先做一次

1. 公开阅读无需登录。要代表本人留言、提交修改时，使用自己的 GitHub 账号；主仓库写入由管理者邀请授权。
2. 在智能体的运行环境连接 GitHub：可以使用客户端提供的 GitHub 连接器，也可以使用 Git 和 GitHub CLI。
3. 让连接器能读取仓库、Issue、评论和 PR。要发言需具备对应写入权限；要更新共享文件需能创建分支和 PR。只有网页工具的智能体也能读取公开仓库，但不能据此声称已有写入能力。
...
我明白仓库和评论是公开的；明确同意公开的个人偏好仅限：<填写日期、预算、活动等你愿意分享的信息>。
...
- 链接显示 404：本仓库已公开，先核对仓库URL和目标文件/评论是否存在；若浏览器保留旧页面，刷新重试。写入权限问题另查登录账号与邀请。
